...
class TestJob(unittest.TestCase):
    ...
    def test_job_get_results(self):
        ...
        res = job.get_results(authentication=anon)
        assert len(res) == 3, \
    ...
    def test_job_get_results_file(self):
        ...
        jobid = "12345"
        outputFormat = "votable"
        outputFile = data_path("result_1.vot")
        job.set_jobid(jobid)
        job.set_output_format(outputFormat)
        job.set_output_file(outputFile)
        responseCheckPhase = DummyResponse()
    ...
    def test_job_save_results(self):
        ...
        responseGetData.set_data(method='POST',
                                 context=None,
                                 body=None,
                                 headers=newHeaders)
        responseGetData.set_status_code(303)
        responseGetData.set_message("See Other")
        redirectResultsJob = DummyResponse()
        redirectResultsJob.set_status_code(200)
        redirectResultsJob.set_message("OK")
        redirectDataFile = data_path('result_1.vot')
